...
import { ContentSafetyAnalyzeDto } from "../agent/dto/content-safety.dto";
import { ContentSafetyStatusDto } from "../agent/dto/content-safety-status.dto";
...
export class ContentSafetyController {
  ...
  @ApiResponse({
    status: 200,
    description: "Batch content safety analysis result.",
  })
  async batchAnalyzeContent(@Body() body: { texts: string[] }) {
    return await Promise.all(
      body.texts.map((text) => this.contentSafety.analyzeText(text)),
    );
  }
  ...
  @ApiResponse({
    status: 200,
    description: "Prompt content safety validation result.",
  })
  ...
  @ApiResponse({
    status: 200,
    description: "Response content safety validation result.",
  })

...
export class LangGraphWorkflowService {
  ...
  async *executeWorkflowStream(
    ...
    this.logger.log("Streaming LangGraph workflow");
    ...
    try {
      ...
      // Use streamEvents to get fine-grained events including LLM tokens
      // Note: This requires LangChain v0.2+ and compatible LangGraph
      // If streamEvents is not available, we fall back to stream() which gives node updates
      ...
      if (typeof workflow.streamEvents === 'function') {
        ...
        for await (const event of stream) {
          ...
          // Map LangGraph events to our Agent events
        ...
        // Fallback to standard stream (node updates)
        const stream = await workflow.stream(initialState);

        for await (const chunk of stream) {
          if (signal?.aborted) return;

          // Chunk contains the state update
          // We can try to extract the last message delta if possible, 
          // but usually it's the full state.
          // For now, just log it.
          // this.logger.debug("Graph chunk", chunk);
        }
      }

    } catch (error: any) {
      this.logger.error(`Workflow streaming failed: ${error.message}`);
      throw error;
    }
  }

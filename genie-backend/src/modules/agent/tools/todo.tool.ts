...
/**
 * Todo Tool
 * Manages a todo list with create, list, complete, delete operations
 */
...
export const createTodoTool = (): DynamicStructuredTool => {
  ...
  return new DynamicStructuredTool({
    name: "todo",
    description:
      "Manages a todo list. Can create, list, complete, and delete todos.",
    schema: z.object({
      operation: z
        .enum(["create", "list", "complete", "delete", "clear"])
        .describe("The todo operation to perform"),
      sessionId: z.string().describe("Session ID for storing todos"),
      title: z.string().optional().describe("Todo title (required for create)"),
      description: z
        .string()
        .optional()
        .describe("Todo description (optional for create)"),
      todoId: z
        .string()
        .optional()
        .describe("Todo ID (required for complete and delete)"),
    }),
    func: async ({
      operation,
      sessionId,
      ...
      try {
        ...
        switch (operation) {
          ...
          case "complete": {
            if (!todoId)
              return "Error: todoId is required for complete operation";
          ...
          case "delete": {
            if (!todoId)
              return "Error: todoId is required for delete operation";

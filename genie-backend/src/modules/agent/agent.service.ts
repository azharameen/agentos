...
/**
 * AgentService
 * Legacy service for direct Azure OpenAI queries
 * Now uses AzureOpenAIAdapter for client pooling (60-70% performance improvement)
 */
@Injectable()
export class AgentService {
  ...
  async queryAgent(
    ...
    if (!this.endpoint || !this.apiKey) {
      ...
      throw new InternalServerErrorException(
        "Azure OpenAI endpoint or API key not configured",
      );
    }
    try {
      ...
      this.logger.error(`Azure OpenAI call failed: ${err}`, err instanceof Error ? err.stack : undefined);
      throw new InternalServerErrorException(
  ...
  private async handleChatCompletion(
    ...
    // PERFORMANCE FIX: Use cached client from adapter (60-70% improvement)
    const client = this.azureAdapter.getOpenAIClient(modelObj.name);
  ...
  private async handleResponseCompletion(
    dto: AgentQueryDto,
    modelObj: any,
  ): Promise<{ answer: string; model: string; raw: any }> {
    ...
    try {
      ...
      this.logger.debug(
        `Full Azure Codex response: ${JSON.stringify(res.data)}`,
      );
      const data = res.data;
      let answer = "";
      ...
      if (
        ...
        if (
          outputArr.length > 0 &&
          outputArr[0] &&
          typeof outputArr[0] === "object" &&
          "text" in outputArr[0] &&
          typeof (outputArr[0] as { text: unknown }).text === "string"
        ) {
      ...
      if (
        ...
        if (
          ...
          "text" in choicesArr[0] &&
          typeof (choicesArr[0] as { text: unknown }).text === "string"
        ) {
      ...
      if (
        ...
        this.logger.error(
          `Azure OpenAI Codex error response: ${JSON.stringify((err.response as { data: unknown }).data)}`,
        );
      }
      throw err;
    }

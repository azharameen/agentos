...
export class WorkflowController {
  ...
  async getWorkflowVersion(@Param('name') name: string, @Param('version') version: string) {
  ...
  async getVersionSnapshots(@Param('name') name: string, @Param('version') version: string) {
  ...
  @ApiResponse({ status: 200, description: 'Comparison of workflow versions with differences.' })
  async compareWorkflowVersions(@Param('name') name: string, @Param('version1') version1: string, @Param('version2') version2: string) {
    return await this.workflowVersioning.compareWorkflowVersions(name, version1, version2);
  }
}

...
export const createCurrentTimeTool = (): DynamicStructuredTool => {
  return new DynamicStructuredTool({
    name: "current_time",
    description:
      "Returns the current date and time in ISO format. Use this when you need to know what time it is now.",
    schema: z.object({
      format: z
        .enum(["iso", "local", "unix"])
        .optional()
        .describe("Output format: iso (default), local, or unix timestamp"),
    }),
    func: async ({
      format = "iso",
    }: {
      format?: "iso" | "local" | "unix";
    }): Promise<string> => {
      const now = new Date();

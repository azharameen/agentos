...
export const createDateCalculatorTool = (): DynamicStructuredTool => {
  return new DynamicStructuredTool({
    ...
    description:
      "Performs date calculations. Can add/subtract days from a date, calculate days between dates, or get day of week.",
    schema: z.object({
      operation: z
      ...
      date: z
        ...
        .optional()
        .describe(
          "Starting date in ISO format (YYYY-MM-DD). Defaults to today if not provided.",
        ),
      days: z
        .number()
        .optional()
        .describe(
          "Number of days to add/subtract (required for add_days and subtract_days)",
        ),
      endDate: z
        .string()
        .optional()
        .describe("End date in ISO format (required for days_between)"),
    }),
    func: async ({
      operation,
      date,
      ...
      try {
        ...
        switch (operation) {
          case "add_days": {
            if (days === undefined)
              return "Error: days parameter is required for add_days operation";
          ...
          case "subtract_days": {
            if (days === undefined)
              return "Error: days parameter is required for subtract_days operation";
            const result = new Date(startDate);
            result.setDate(result.getDate() - days);
          ...
          case "days_between": {
            if (!endDate)
              return "Error: endDate parameter is required for days_between operation";

...
export const createFilesystemTool = (): DynamicStructuredTool => {
  ...
  return new DynamicStructuredTool({
    name: 'filesystem',
    description: 'Reads, writes, lists, and manipulates files and directories. Supports read, write, list, exists, delete operations.',
    schema: z.object({
      operation: z.enum(['read', 'write', 'list', 'exists', 'delete', 'create_dir']).describe('The filesystem operation to perform'),
    ...
    func: async ({ operation, filePath, content }: {
      ...
      try {
        ...
        switch (operation) {
          ...
          case 'write': {
            if (!content) return 'Error: content parameter is required for write operation';
...
export const FILESYSTEM_TOOL_METADATA = {
  name: 'filesystem',

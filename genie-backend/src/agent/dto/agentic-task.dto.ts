...
export class AgenticTaskDto {
  @ApiProperty({
    ...
    example: 'Calculate 25 * 4 and then tell me what day it will be 10 days from now',
  })
  @IsString()
  prompt: string;
  ...
  @ApiPropertyOptional({
    description: 'Enable only specific tool categories (e.g., math, string, web)',
  ...
  @ApiPropertyOptional({
    description: 'Use simple mode (direct LLM call without agent orchestration)',
    example: false,
  })
  @IsOptional()
  @IsBoolean()
  ...
  @ApiPropertyOptional({
    description: 'Use LangGraph workflow instead of LangChain AgentExecutor (enables advanced graph-based reasoning)',
    example: false,
  })
  @IsOptional()
  @IsBoolean()
...
export class AgenticResponseDto {
  @ApiProperty({
    ...
    example: 'The result of 25 * 4 is 100. Ten days from now will be November 17, 2025.',
  })
  output: string;
